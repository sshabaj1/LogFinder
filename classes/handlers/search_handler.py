...
class SearchHandler():
    ...
    def search_logs(path, searchword):
        ...
        try:
            ...
            is_error = True
            LogHandler.critical_log(
                'SearchHandler', function_name, 'OS Error', e)
                        ...
                        for search_word in search_words:
                            if search_word in line:
                                ...
                                LogHandler.debug_log(
                                    'SearchHandler', function_name, 'matched_files', matched_files)

            # iterate through all file
            for file in os.listdir():
                LogHandler.info_log(
                    'SearchHandler', function_name, 'enterd in loop/file: ', file)
                if file.endswith(StaticVariables.LOG_EXTENTION):
                    file_path = f"{path}\{file}"
                    LogHandler.debug_log(
                        'SearchHandler', function_name, 'file_path', file_path)
