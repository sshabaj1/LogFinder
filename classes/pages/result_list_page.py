...
class ResultListPage(tk.Frame):
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        self.canvas = tk.Canvas(self, bg='DeepSky Blue2',
                                width=668, height=550, relief="sunken")
        ...
        def on_double_click(event):
            ...
            LogHandler. debug_log(
                self, function_name, 'clicked_record', clicked_record.replace('\\', '/'))
            master.open_file(clicked_record)

        for file in file_list:
            LogHandler. debug_log(
                self, 'on_double_click/for loop/', 'file', file)
            correct_path = file.replace('\\', '/')
            LogHandler. debug_log(
                self, 'on_double_click/for loop/', 'correct_path', correct_path)
            file_name = correct_path.rsplit('/', 1)[1]
            LogHandler. debug_log(
                self, 'on_double_click/for loop/', 'file_name', file_name)
        ...
        back_button = tk.Button(self, image=self.back_image,
                                borderwidth=0, bg='DeepSky Blue2', command=handle_back)
        back_button_canvas = self.canvas.create_window(
            30, 480, anchor="nw", window=back_button)
